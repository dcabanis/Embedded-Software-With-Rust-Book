...
fn main() -> ! {
    ...
    // Enable the clock for GPIO Port C using a safe read-modify-write
    rcc.apb2enr().modify(|_, w| w.iopcen().set_bit());

    // Configure PC13 as a push-pull output using the PAC's safe `modify` API.
    // This avoids using an `unsafe` block and magic numbers.
    gpioc.crh().modify(|_, w| {
        w.mode13()
            .output2() // Set mode to output, 50MHz
            .cnf13()
            .push_pull() // Set configuration to push-pull
    });

    loop {
        // Turn on LED (set PC13 low for an active-low LED)
        gpioc.bsrr().write(|w| w.br13().set_bit());
        // Wait for the SysTick timer to wrap (1 second)
        while !syst.has_wrapped() {}

        // Turn off LED (set PC13 high for an active-low LED)
