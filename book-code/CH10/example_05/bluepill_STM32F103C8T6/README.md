...
This example covers *cbindgen* to generate a C header automatically from Rust source on the STM32F103C8T6 (Blue Pill).
...
matches what the Rust source promises, the Blue Pill's PC13 LED blinks
steadily; a mismatch (meaning the generated header disagrees with the
compiled library) latches the LED on solid instead.
...
The onboard LED blinks at a steady ~4 Hz, confirming
`checksum(payload, 4) == 0xA0`, `checksum(payload, 0) == 0`, and
`checksum(NULL, 4) == 0` all matched the header cbindgen generated.
